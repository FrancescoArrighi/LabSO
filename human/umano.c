...
Esempio operazioni (dove 71 è id di un bulb): \n \
  -> 71 bulb get info \n \
  -> 71 bulb open\n \
  -> exit \n \
"
...
int main(){
  ...
  while(flag){
    ...
    if(buf_w[0] != '\n'){ // risolve il problema di segmentation fault quando vado a capo non scrivendo nulla
      ...
      if(strcmp(cmd[0], "exit") == 0){
      ...
      else if(is_integer(cmd[0]) == TRUE){
        if(n_arg == 3){ //Azioni dirette sugli interruttori (atomiche)
        ...
        else if((n_arg == 5) && (strcmp(cmd[2], "set") == 0) && (is_integer(cmd[4]) == TRUE)) { //Richieste di setting
          sprintf(fifo_w, "/tmp/D_%s_R", cmd[0]);
          printf("%s\n", fifo_w);
          fd = open(fifo_w, O_WRONLY);
          strcpy(risposta.msg_text, "\0");

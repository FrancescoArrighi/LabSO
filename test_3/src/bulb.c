...
void bulb(int id, int recupero, char * nome){
  ...
  if((idf = fork()) < 0){
    ...
    while (TRUE) {
      ...
      else if(codice == MSG_SETSTATO && controllo_bulb(msg, id)){ //Inverto il mio stato
        printf("Bulb:Invio ack p\n");
        if ((status == FALSE) && (atoi(msg[MSG_SETSTATO_VAL]) == TRUE)) {
          status = atoi(msg[MSG_SETSTATO_VAL]);
          t_start = time(NULL);
        }
        else {
          status = atoi(msg[MSG_SETSTATO_VAL]);

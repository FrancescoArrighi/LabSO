...
void swtch(char ** cmd, int n, int_list * figli, int queue, int deposito){
  int flag = TRUE, destinatario;
  msgbuf richiesta;
  int invia_controllla = FALSE;
  if(n == 4){
    printf(" swtch: 1 %s, %d, %d, %d\n", cmd[1],is_integer(cmd[1]),cmd[2] != NULL , cmd[3] != NULL);
    if(is_integer(cmd[1]) == TRUE && cmd[2] != NULL && cmd[3] != NULL){
      ...
      if(invia_controllla == TRUE){
        richiesta.msg_type = NUOVA_OPERAZIONE;
        invia_broadcast(&richiesta, figli);
        msgsnd(deposito, &richiesta, sizeof(richiesta.msg_text), 0);
        int i, flag2 = TRUE, flag3 = FALSE, dim_msg;
        msgbuf risposta_figli;
        char ** msg_risp_f;
        for(i = figli->n + 1; i > 0 && flag2; i--){
          if(leggi(queue, &risposta_figli, 2, 2)){
        ...
        if(flag3 == TRUE){
          printf("Switch eseguito con successo\n" );
        }
        else{
          printf("Errore nell'esecuzione della Switch\n" );
        }
      }
    }
    else{
  ...
  if(flag == FALSE){
    printf("\nErrore campi: switch <type dispositivo>\n <type dispositivo>: \"hub\", \"timer\", \"bulb\", \"window\", \"fridge\"\n");
  }
}
    ...
    while (flag && getline(&str, &dim_str, stdin) >= 0) {
        ...
        if(n > 0){
          if(strcmp(cmd[0], "list") == 0){
            ...
            swtch(cmd, n, figli, my_queue, queue_deposito);
          }
